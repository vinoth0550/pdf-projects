...
# # this code is working well this is for switch over 7 templates and after the 7 th template it will stop and return json response.
...
# # Convert dict → object for your templates
# class Obj:
#     def __init__(self, d):
#         for k, v in d.items():
#             setattr(self, k, v)
...
#         # Check if template is async or sync
#         if inspect.iscoroutinefunction(selected_template):
#             return await selected_template(obj)   # async function
#         else:
#             return selected_template(obj)         # normal function
...
# Below code is for json response with and save in a folder and each template retuern a download link

# Also use 4 digit uuid to avoiding overwriting files with same name.
